...
import { IconSearch, IconPlus, IconDownload, IconArrow } from '@/components/Icons';
...
export default function ContractsPage() {
  ...
  const filtered = SAMPLE_CONTRACTS.filter((c) => {
    const matchSearch = !search || c.propertyName.includes(search) || c.tenantName.includes(search) || c.contractNo.includes(search);
    ...
    <AppLayout title="契約書管理">
          ...
          <table className="data-table">
            <thead>
              <tr>
                ...
                <th>ステータス</th><th>作成日</th><th>更新日</th><th></th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((c) => (
                <tr key={c.id}>
                  <td><span style={{ fontFamily: 'monospace', fontSize: 12, fontWeight: 600, color: 'var(--navy)' }}>{c.contractNo}</span></td>
                  <td>
                  ...
                  <td><span style={{ fontSize: 12, background: 'var(--earth-pale)', padding: '3px 8px', borderRadius: 6, fontWeight: 600 }}>{TYPE_LABELS[c.type]}</span></td>
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={8} style={{ textAlign: 'center', padding: '40px', color: 'var(--text-muted)' }}>契約書が見つかりませんでした</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <div style={{ display: 'flex', gap: 16, marginTop: 16, flexWrap: 'wrap' }}>
        {Object.entries(STATUS_LABELS).map(([k, v]) => (
          <span key={k} className={`status-badge status-${k}`}>{v}</span>
        ))}
      </div>
    </AppLayout>
  );
