...
import {
  ArrowLeft, FileText, Clock, Shield, Download, Send,
  CheckCircle, AlertCircle, PenLine, Lock, Wifi
} from 'lucide-react';
...
export default function ContractDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!contract) {
    return (
      <AppLayout title="契約書詳細">
        ...
        <Link href="/contracts" className="btn btn-outline" style={{ marginTop: 16 }}>
          ← 一覧に戻る
        </Link>
      </AppLayout>
    );
  ...
  return (
    <AppLayout title={`契約書詳細：${contract.contractNo}`}>
      {/* Back + title */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20, flexWrap: 'wrap' }}>
        ...
        <div>
          <div style={{ fontSize: 18, fontWeight: 800 }}>{contract.propertyName}</div>
          <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
            {contract.contractNo} / {TYPE_LABELS[contract.type]}
          </div>
        </div>
        <span className={`status-badge ${statusClass[contract.status]}`} style={{ marginLeft: 'auto' }}>
      ...
      {tab === 'overview' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
          <div className="card">
            <div className="card-header"><div className="card-title">物件情報</div></div>
          ...
          <div className="card">
            <div className="card-header"><div className="card-title">当事者情報</div></div>
            <div className="card-body">
              <table className="data-table">
                <tbody>
          ...
          <div className="card" style={{ gridColumn: '1 / -1' }}>
            <div className="card-header">
              <div className="card-title"><Shield size={14} style={{ display: 'inline', marginRight: 6 }} />セキュリティ情報</div>
            </div>
            <div className="card-body">
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
                {[
                  { label: 'タイムスタンプ', value: contract.signedAt ? '付与済み' : '未付与', ok: !!contract.signedAt },
                  { label: '署名者IPアドレス', value: contract.signerIp || '未署名', ok: !!contract.signerIp },
                  { label: '改ざん検証', value: contract.status === 'completed' ? '検証済み' : '待機中', ok: contract.status === 'completed' },
                  { label: '電子帳簿保存法', value: '準拠済み', ok: true },
                  { label: '通信暗号化', value: 'TLS 1.3', ok: true },
                  { label: '保存期間', value: '7年間（法定）', ok: true },
                ].map((item) => (
                  <div key={item.label} style={{
                    padding: '12px 14px',
                    background: item.ok ? '#f0fdf4' : 'var(--bg-base)',
                    borderRadius: 'var(--radius)',
                    border: `1px solid ${item.ok ? '#bbf7d0' : 'var(--border)'}`,
                  }}>
                    <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 4 }}>{item.label}</div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: item.ok ? 'var(--green-ok)' : 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: 4 }}>
                      {item.ok && <CheckCircle size={12} />}
                      {item.value}
                    </div>
                  </div>
                ))}
      ...
      {tab === 'preview' && (
        <div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginBottom: 16 }}>
            <button className="btn btn-outline btn-sm"><Download size={14} /> PDF ダウンロード</button>
            {contract.status === 'pending' && (
              <button className="btn btn-primary btn-sm"><Send size={14} /> リマインド送信</button>
            )}
          </div>
          <div className="contract-preview">
            <h1>{TYPE_LABELS[contract.type]}</h1>
            <div className="contract-date">
              契約番号：{contract.contractNo}<br />
            ...
            <div className="contract-parties">
              <div className="party-box">
                ...
                <div className="party-name">物件オーナー</div>
              </div>
              <div className="party-box">
                <div className="party-label">借主（乙）</div>
                <div className="party-name">{contract.tenantName}</div>
                <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{contract.tenantEmail}</div>
              </div>
            </div>
            ...
            <div className="seal-area">
              <div className="seal-box">
                <div className={`seal-circle ${contract.status === 'completed' ? 'signed' : ''}`}>
                  {contract.status === 'completed' ? '✓' : '甲'}
                </div>
                <div className="seal-label">貸主</div>
              </div>
              <div className="seal-box">
                <div className={`seal-circle ${['signed', 'completed'].includes(contract.status) ? 'signed' : ''}`}>
                  {['signed', 'completed'].includes(contract.status) ? '✓' : '乙'}
                </div>
                <div className="seal-label">借主</div>
              </div>
              <div className="seal-box">
                <div className="seal-circle" style={{ borderColor: 'var(--navy)' }}>宅</div>
                <div className="seal-label">宅建士</div>
              </div>
            </div>

            {contract.signedAt && (
              <div style={{ marginTop: 24, padding: '12px', background: '#f0fdf4', borderRadius: 8, fontSize: 11, color: 'var(--green-ok)' }}>
        ...
        <div style={{ maxWidth: 600, margin: '0 auto' }}>
          {signed || contract.status === 'completed' || contract.status === 'signed' ? (
            <div style={{ textAlign: 'center', padding: '40px 0' }}>
              <div style={{ width: 72, height: 72, borderRadius: '50%', background: 'var(--green-ok)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32, margin: '0 auto 20px' }}>✓</div>
              <div style={{ fontSize: 20, fontWeight: 800, marginBottom: 8 }}>電子署名が完了しています</div>
              <div style={{ fontSize: 14, color: 'var(--text-muted)' }}>
                {contract.signedAt ? `署名日時：${formatDate(contract.signedAt)}` : '署名済み'}
              </div>
            </div>
          ) : (
            <>
              <div className="alert alert-warn" style={{ marginBottom: 20 }}>
                <AlertCircle size={14} />
                <div>
                  <strong>電子署名の前に必ずお読みください</strong><br />
                  <span style={{ fontSize: 12 }}>署名することで契約内容に同意したことになります。内容をよく確認してから署名してください。</span>
                </div>
              </div>

              {/* Security status */}
              <div style={{ background: 'var(--navy)', borderRadius: 'var(--radius)', padding: '12px 16px', marginBottom: 20, display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)' }}>セキュリティ状態</span>
                {[
                  { label: 'TLS暗号化', icon: <Lock size={10} /> },
                  { label: 'タイムスタンプ', icon: <Clock size={10} /> },
                  { label: '接続確認', icon: <Wifi size={10} /> },
                ].map(item => (
                  <span key={item.label} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, color: '#4ade80' }}>
              ...
              <div className="card">
                <div className="card-header">
                  <div className="card-title"><PenLine size={14} style={{ display: 'inline', marginRight: 6 }} />電子署名パッド</div>
                  <button className="btn btn-outline btn-sm" onClick={clearSig}>クリア</button>
                </div>
                <div className="card-body">
                  <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 12 }}>
                    以下の枠内にマウスまたは指でサインしてください。
                  </p>
                  <canvas
                    ref={canvasRef}
                    width={520}
                  ...
                  <div style={{ marginTop: 16 }}>
                    <label style={{ display: 'flex', alignItems: 'flex-start', gap: 8, fontSize: 12, cursor: 'pointer' }}>
                      ...
                      <span>
                        契約書の内容を確認し、宅地建物取引業法に基づく重要事項説明を受けた上で、
                        本契約内容に同意し電子署名を実施します。
                        本署名は法的効力を有することを理解しています。
                      </span>
                    </label>
                  </div>
                </div>
      ...
      {tab === 'audit' && (
        <div className="card">
          <div className="card-header">
            <div className="card-title"><Shield size={14} style={{ display: 'inline', marginRight: 6 }} />操作・監査ログ</div>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  {entry.ip && (
                    <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2, fontFamily: 'monospace' }}>
                      IP: {entry.ip} {entry.userId && `| ユーザー: ${entry.userId}`}
                    </div>
                  )}
                </div>
                <div style={{
                  width: 8, height: 8, borderRadius: '50%', flexShrink: 0, marginTop: 4,
                  background: entry.event.includes('署名') ? 'var(--green-ok)' :
                    entry.event.includes('送信') ? 'var(--blue-info)' :
                    entry.event.includes('チェック') ? '#7c3aed' : 'var(--border-strong)',
                }} />
              </div>
            ))}
          </div>
